...
DefaultCategoryDataset dataset = new DefaultCategoryDataset();
 ...
 if(!zyList.isEmpty()){
 	for(int i=0;i<zyList.size();i++){
 		...
 		float str=cb.getFloat("select sum(hkje) from hk where bmmc='"+list2.get(1).toString()+"' and sj>='"+ksj+"'  and sj<='"+esj+"'");
 		//System.out.println(ksj.substring(0,4));
 		float ndys=cb.getFloat("select ysje from ndys where ysnd='"+ksj.substring(0,4)+"'");
 		float hdys=cb.getFloat("select sum(ysje) from hdys where bmmc='"+list2.get(1).toString()+"' and sj>='"+ksj+"'  and sj<='"+esj+"'");
 		float jk=cb.getFloat("select sum(jkje) from jk where bmmc='"+list2.get(1).toString()+"' and sj>='"+ksj+"'  and sj<='"+esj+"'");
 		float yy=str-(ndys+hdys+jk);
 		dataset.addValue(yy, " ", list2.get(1).toString());
 	}
 }
...
JFreeChart chart = ChartFactory.createBarChart3D("收益信息统计图","部门","金额",dataset,PlotOrientation.VERTICAL,false,false,false);
